...
void resetvalue(void) {
  val = 1234567;
  int nval = N / size;
  for (int j = 0;j < rank*nval; j++){
    for(int i = m/sizeof(numb); i > 0; i--){
      next_rand();
    }
  }
}

obj new_random_obj(void) {
  obj o = malloc(m);
  obj o2 = o;
  // fill with random data
  for (int64_t i = m/sizeof(numb); i > 0;i--) {
    *o2 = next_rand();
    o2++;
  }
  return o;
...
// HashTable size in total
numb hashlen;
numb collisions;

// HashTable size per process
numb nobj;
...
int main(int argc, char *argv[]) {
  ...
  m *= sizeof(numb);
  k = atol(argv[3]);
  printf("N=%ld, m=%ld, k=%ld\n", N, m, k);

  // hash_table size is 2*N+1
  inithash(N);
